...
function joinRoom(socket, roomID) {
  ...
  if (!roomFound) {
    //no rooms in queue
    if (notFullRooms.head == null) createRoom();
  ...
  io.to(room.id).emit(
    'playerConnection',
    socket.id + ' has joined room: ',
    roomID,
    PLAYERSPERROOM - room.numInRoom - 1
  );
...
function leaveFullRoom(socket) {
  ...
  //a player leaving a full room with no game going on means players need to reready
    ...
    io.to(room.id).emit(
      'playerConnection',
      'Player ' + socket.id + ' has left room: ',
      room.id,
      PLAYERSPERROOM - room.numInRoom
    );
  ...
  while (notFullRooms.next()) {
    if (notFullRooms.current.id == roomID) {
      ...
      io.to(roomID).emit(
        ...
        'Player ' + socket.id + ' has left room: ',
        roomID,
        PLAYERSPERROOM - notFullRooms.current.numInRoom
      );
  ...
  this.update = function() {
    ...
    for (var i = 0; i < this.tanks.length; i++) {
      for (var j = 0; j < this.tanks[i].bullets.length; j++) {
        ...
        if (
          this.tanks[i].bullets[j].hitBox.pos.x < 0 ||
...
function Tank(socket) {
  ...
  this.move = function(Idirection) {
    ...
    if (Idirection == UP) {
      ...
      if (SAT.testPolygonPolygon(this.hitBox, CVS_TOP) || checkTankcollision(this)) {
        this.hitBox.pos.y += -moveAmt;
      }
      //Right
    } else if (Idirection == RIGHT) {
      this.hitBox.pos.x += moveAmt;
      if (SAT.testPolygonPolygon(this.hitBox, CVS_RIGHT) || checkTankcollision(this)) {
        this.hitBox.pos.x += -moveAmt;
      }
      //Down
    } else if (Idirection == DOWN) {
      this.hitBox.pos.y += -moveAmt;
      if (SAT.testPolygonPolygon(this.hitBox, CVS_BOTTOM) || checkTankcollision(this)) {
        this.hitBox.pos.y += moveAmt;
      }
      //Left
    } else if (Idirection == LEFT) {
      this.hitBox.pos.x += -moveAmt;
      if (SAT.testPolygonPolygon(this.hitBox, CVS_LEFT) || checkTankcollision(this)) {
        this.hitBox.pos.x += moveAmt;
      }
    }
  ...
  this.fireBullet = function() {
    ...
    if (this.shootCooldown <= 0) {
      ...
      if (this.direction == UP) {
        ...
        this.bullets[this.bullets.length] = new Bullet(
          this.hitBox.pos.x + INI_TANKHEIGHT / 2 + BULLETSIZE,
          this.hitBox.pos.y,
          bulletSpeed,
          0
        );
      ...
      switch (i) {
        case 0: //Top left
          return new SAT.Vector(
            getRandom(halfTankSize, CVSWIDTH / 3 - halfTankSize),
            getRandom((2 * CVSHEIGHT) / 3 + halfTankSize, CVSHEIGHT - halfTankSize)
          );
        case 1: //Middle right
          return new SAT.Vector(
            getRandom((2 * CVSWIDTH) / 3 + halfTankSize, CVSWIDTH - halfTankSize),
        ...
        case 3: //Top middle
          return new SAT.Vector(
            getRandom(CVSWIDTH / 3 + halfTankSize, (2 * CVSWIDTH) / 3 - halfTankSize),
            getRandom((2 * CVSHEIGHT) / 3 + halfTankSize, CVSHEIGHT - halfTankSize)
          );
        case 4: //Bottom right
          return new SAT.Vector(
